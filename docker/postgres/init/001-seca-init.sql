...
CREATE TYPE active_model_type AS ENUM ('gemini_freetier', 'local_8B', 'openai_4_mini', 'openai_4_regular', 'openrouter');
...
CREATE TABLE creative_relationships (
  ...
  love_hate_score SMALLINT NOT NULL DEFAULT 0 CHECK (love_hate_score BETWEEN -100 AND 100),
...
CREATE INDEX idx_creative_moods_session
...
CREATE INDEX idx_creative_beliefs_active
  ON creative_beliefs (session_id, status, created_dttm);

CREATE TABLE creative_belief_runs (
  run_id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  session_id INTEGER NOT NULL,
  user_id INTEGER NOT NULL,
  status VARCHAR(20) NOT NULL CHECK (status IN ('running', 'completed', 'failed')),
...
CREATE INDEX idx_creative_belief_runs_session
...
CREATE INDEX idx_creative_goals_active
  ON creative_goals (session_id, status, priority, updated_dttm DESC);

CREATE TABLE creative_goal_steps (
...
CREATE INDEX idx_creative_goal_steps_goal
  ON creative_goal_steps (goal_id, status, sequence_num, step_id);

CREATE TABLE creative_goal_events (
...
CREATE INDEX idx_creative_goal_events_goal
  ON creative_goal_events (goal_id, created_dttm DESC);

CREATE TABLE creative_goal_runs (
  run_id INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  session_id INTEGER NOT NULL,
  user_id INTEGER NOT NULL,
  status VARCHAR(20) NOT NULL CHECK (status IN ('running', 'completed', 'failed')),
...
CREATE INDEX idx_creative_goal_runs_session
  ON creative_goal_runs (session_id, started_dttm DESC);
